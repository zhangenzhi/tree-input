...
from model.vit import create_vit_base
...
def setup_logging(output_dir):
    ...
    logging.basicConfig(
        filename=os.path.join(output_dir, "vit_train.log"),
        level=logging.INFO,
...
def main():
    parser = argparse.ArgumentParser(description="ViT-B ImageNet Training (DDP)")
    parser.add_argument("--data_dir", type=str, required=True)
    parser.add_argument("--output", type=str, default="./output/vit_b")
    ...
    # Model
    model = create_vit_base(num_classes=1000, pretrained=False)
    ...
    warmup_scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer, start_factor=1e-4, total_iters=args.warmup_epochs
    )
    ...
    for epoch in range(args.num_epochs):
        ...
        # Step LR per iteration is handled inside; here step per epoch as fallback
        lr_scheduler.step()
